...
class CreateContractsTable extends Migration
{
    ...
    {												
        Schema::create('contracts', function (Blueprint $table) {
            ...
            $table->text('adjudicatarios')->index(); // 502496878 - CONSTRUÇÕES PRAGOSA, S.A.
            $table->date('dataPublicacao'); // 29/06/2016
            $table->date('dataCelebracaoContrato')->index(); // 31/05/2016
            $table->decimal('precoContratual', 16, 2)->index(); // 2605654.08
            ...
            // add indexes to columns we search regularly as the table will get really large
            $table->index(['adjudicatarios', 'precoContratual']);
            $table->index(['adjudicatarios', 'dataCelebracaoContrato']);
            $table->index(['dataCelebracaoContrato', 'precoContratual']);
        });
    }
